  ...
  if (status === 'authenticated') {
    return (
      <>
        ...
        <button
          onClick={() => signOut()}
          className="primary rounded-full text-white px-8 py-2">
          Logout
        </button>
      </>
    );
  ...
  return (
    <header>
      <div className="flex items-center md:hidden justify-between">
        <Link className="text-[#f13a01] underline font-semibold text-2xl" href={'/'}>
          NORTHEAST CANTEEN
        </Link>
      ...
      <Link className="text-[#f13a01] text-2xl underline whitespace-nowrap" href="/">
          NORTHEAST CANTEEN
        </Link>
        <Link href={"/"}>Home</Link>
        <Link href={"/menu"}>Menu</Link>
      ...
      {/* {status === 'authenticated' && (
        ...
        className="rounded-full primary px-8 py-2">
        Logout
        </button>
        </>
      )}
...
<Link href={'/cart'} className="relative" >
  <ShoppingCart/>
  {cartProducts.length > 0 && (
    <span className="absolute -top-2 -right-4 bg-[#f13a01] text-white text-xs py-1 px-1 rounded-full leading-3">{cartProducts.length}</span>
  )}
</Link>
